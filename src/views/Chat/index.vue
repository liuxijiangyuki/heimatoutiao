...
export default {
  ...
  data () {
    return {
      ...
      list: [
        ...
        { name: 'me', msg: '我是编程小王子' },
        { name: 'xs', msg: 'hi，你好！我是小思' },
        { name: 'me', msg: '我是编程小王子' }, { name: 'xs', msg: 'hi，你好！我是小思' },
        { name: 'me', msg: '我是编程小王子' }
      ],
      socket: null
    }
  ...
  created () {
    ...
    this.socket.on('message', data => { // 把服务器传过来的消息传递到list里
      this.list.push({
        name: 'xs',
        msg: data.msg
      })
  ...
  methods: {
    sendMsg () {
      if (this.word.length.trim === 0) return
      this.socket.emit('message', { // 把消息发送到websocket服务器
        msg: this.word,
